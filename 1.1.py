    ...
    def run(self, state: TradingState):
        ...
        if state.timestamp < (100 * 100):
            ...
            for product in products:
                ...
                short_bid_sma = np.mean(
                    product_prices["BID"][-int(len(product_prices["BID"]) / 4) :]
                )
                long_bid_sma = np.mean(product_prices["BID"])
                if short_bid_sma > long_bid_sma and self.short_sma_above is False:
                    result[product] = self.fill_orders(
                        product, state.order_depths[product], 5, buy=1
                    )
                    self.short_sma_above = True
                elif short_bid_sma < long_bid_sma:
